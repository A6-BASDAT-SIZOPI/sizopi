...
export default function BookingAtraksiPage() {
  ...
  const [atraksi, setAtraksi] = useState<AtraksiDetail | null>(null)
  const [formData, setFormData] = useState({
    tanggal_kunjungan: new Date(),
    jumlah_tiket: 1,
  })
  const [errors, setErrors] = useState<Record<string, string>>({})
  const namaAtraksi = decodeURIComponent(params.nama as string)
  ...
  }, [user, loading, userRole, namaAtraksi])

  const fetchAtraksiDetail = async () => {
    try {
      setIsLoading(true)

      const response = await fetch(`/api/reservasi/atraksi/${encodeURIComponent(namaAtraksi)}`)
      if (!response.ok) throw new Error("Gagal memuat detail atraksi")
      ...
      toast({
        title: "Error",
        description: "Gagal memuat detail atraksi: " + error.message,
        variant: "destructive",
      })
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!response.ok) {
        // ⚠️ Show the trigger’s error message in an alert
        ...
        <div className="container mx-auto py-8">
          <div className="max-w-2xl mx-auto bg-white shadow-md rounded-lg overflow-hidden">
            <div className="p-6 text-center">
              <p className="mb-4">Atraksi tidak ditemukan</p>
              <Button asChild>
      ...
      <div className="container mx-auto py-8">
        <div className="max-w-2xl mx-auto bg-white shadow-md rounded-lg overflow-hidden">
          <div className="border-b p-4 flex justify-between items-center">
            <h1 className="text-xl font-bold">FORM RESERVASI ATRAKSI</h1>
          </div>
          ...
          <form onSubmit={handleSubmit} className="p-6">
            <div className="space-y-6">
              <div>
                ...
                <div className="mt-1 p-2 border rounded-md bg-gray-50">{atraksi.nama_atraksi}</div>
              </div>
              ...
              <div>
                ...
                <div className="flex items-center mt-1 p-2 border rounded-md bg-gray-50">
                  ...
                  <span>{formatTime(atraksi.jadwal)}</span>
                </div>
              </div>
