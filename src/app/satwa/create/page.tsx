...
export default function CreateSatwaPage() {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        console.log("Data yang dikirim ke API:", dataToSubmit);
        ...
        try {
            ...
            if (!response.ok) {
                let errorMsgToShow = `Gagal menambahkan data. Status: ${response.status}`;
                try {
                    const errorData = await response.json();
                    errorMsgToShow = errorData.message || JSON.stringify(errorData);
                } catch (jsonError) {
                    const textError = await response.text();
                    console.error("Server returned non-JSON for POST error:", textError.substring(0, 200));
                }
                console.error("Error adding data (client-side):", errorMsgToShow);
